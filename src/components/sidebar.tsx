    ...
    <>
      ...
      <div className="lg:hidden fixed top-0 left-0 right-0 z-50 bg-white border-b border-gray-100 px-4 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="font-bold text-gray-900 text-sm">ClicaPet</span>
        </div>
      ...
      <aside className={`fixed lg:static z-50 top-0 left-0 h-full w-64 bg-white border-r border-gray-100 p-5 flex flex-col transition-transform lg:translate-x-0 ${open ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex items-center gap-2.5 mb-8">
          <div className="w-9 h-9 bg-teal-600 rounded-xl flex items-center justify-center">
            <span className="text-white font-bold text-lg">C</span>
          </div>
          <div>
            <p className="font-bold text-gray-900 leading-tight">ClicaPet</p>
            <p className="text-[11px] text-teal-600 font-medium">Painel da Clinica</p>
          </div>
        </div>
        <nav className="flex-1 space-y-1">
          {links.map((link) => {
            ...
            return (
              ...
              >
                <span className={isActive ? 'text-teal-500' : 'text-gray-400'}>{link.icon}</span>
                {link.label}
              </Link>
            )
          })}
        ...
        <button
          ...
          className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors mt-4"
        >
